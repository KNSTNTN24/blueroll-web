  ...
  const createMutation = useMutation({
    mutationFn: async (values: FormValues) => {
      ...
      const items = values.items.map((item, index) => ({
        ...
        sort_order: index,
        min_value: item.item_type === 'temperature' ? (item.min_value ?? null) : null,
        max_value: item.item_type === 'temperature' ? (item.max_value ?? null) : null,
        unit: item.item_type === 'temperature' ? (item.unit || null) : null,
      }))
    ...
    <div className="space-y-6">
      <div>
        ...
        <PageHeader
          ...
          description="Define a reusable checklist template for your team"
        />
      </div>
