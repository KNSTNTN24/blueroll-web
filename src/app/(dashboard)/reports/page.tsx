...
export default function ReportsPage() {
  ...
  function exportPDF() {
    ...
    // Every completed check, expanded with all its recorded entries (chronological)
    const ordered = filtered.slice().sort((a, b) => new Date(a.completed_at).getTime() - new Date(b.completed_at).getTime())
    const sections = ordered.map((c) => {
      const resp = (c.responses ?? []).slice().sort((a, b) => (a.item?.sort_order ?? 0) - (b.item?.sort_order ?? 0))
      const flagged = resp.filter((r) => r.flagged).length
      const head = `<div class="eyebrow">${esc(c.template?.name ?? 'Checklist')} · ${esc(siteName(c.site_id))} · ${format(parseISO(c.completed_at), 'd MMM yyyy, HH:mm')}</div>`
      const signed = `<div class="signed"><span>Signed: ${esc(c.completed_by_profile?.full_name ?? '—')}</span><span>${resp.length ? `${resp.length - flagged}/${resp.length} items${flagged ? ` · ${flagged} flagged` : ''}` : 'completed'}</span></div>`
      if (!resp.length) return `<div class="sec">${head}<div class="norec">Marked complete — no line entries recorded.</div>${signed}</div>`
      const itemRows = resp.map((r) => `<tr><td>${esc(r.item?.name ?? 'Item')}</td><td class="num">${esc(reading(r.value))}</td><td class="res ${r.flagged ? 'amber' : 'green'}">${r.flagged ? 'Flagged' : 'Pass'}</td><td class="note">${esc(r.notes ?? '')}</td></tr>`).join('')
      return `<div class="sec">${head}<table class="items"><thead><tr><th>Check item</th><th class="num">Reading</th><th class="res">Result</th><th class="note">Notes / corrective action</th></tr></thead><tbody>${itemRows}</tbody></table>${signed}</div>`
    }).join('')
...
<style>
  ...
  table.items td.note,table.items th.note{text-align:left;font-size:11.5px;color:#6b7280;font-weight:400}
  .norec{padding:11px 2px;color:#9aa0a8;font-size:12px;font-style:italic}
</style></head><body>
  <div class="hd">
  ...
  <div class="block"><h2>Records — every check, expanded (${filtered.length})</h2>${sections}</div>
