...
### Список
- Таблица: название, теги (бейджи), аллергены (бейджи), dietary labels, статус (Active/Inactive)
- Фильтры: поиск по тексту, фильтр по тегу
- Теги: per-business, задаются самим бизнесом (например Starters, Mains, Desserts) — рецепт может иметь несколько тегов
...
### Создание рецепта
- Поля: название, описание, теги, инструкции, метод, температура, время, единица
...
- Таб Recipes: таблица активных рецептов с тегами, аллергенами, dietary, статусом
- Фильтр по тегу + опциональный режим "group by tags": блюда сгруппированы по тегам, рецепт с несколькими тегами появляется под каждым своим тегом
...
- Два режима: Card view (плоский список) и Matrix view (таблица)
- Поиск по названию рецепта
- Сортировка: по имени, кол-ву аллергенов
- Экспорт: CSV и PDF
