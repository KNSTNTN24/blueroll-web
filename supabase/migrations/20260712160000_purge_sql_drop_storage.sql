-- Storage purging is no longer done via raw SQL against storage.objects:
-- deleting the metadata row only orphans the physical blob in S3, and it
-- required bypassing the platform's protective BEFORE DELETE trigger via
-- `set local storage.allow_delete_query`. It also skipped the `videos`
-- bucket entirely. Physical-file purging now happens in the
-- `purge-deleted-businesses` edge function via the Storage API's
-- `remove()`, which deletes both the blob and the metadata row correctly
-- for every bucket. This function goes back to purging DB rows + auth
-- users ONLY.
...
-- try_uuid() was only ever used to guard the storage.objects delete that
-- lived in this function; that delete is gone, so the helper is now dead
-- code. Drop it.
